...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ─── Ethereum ────────────────────────────────────────────────────
    {
        ...
        println!("[ETH]  Pubkey:   {} (compressed {}B)", hex::encode(&signer.public_key_bytes()[..8]), signer.public_key_bytes().len());
        println!("[ETH]  Sig v={}, valid={}", sig.v, verifier.verify(b"hello world", &sig)?);
    }
    ...
    // ─── Bitcoin Schnorr ─────────────────────────────────────────────
    {
        ...
        println!("[BTC]  x-only:   {} ({}B)", hex::encode(&signer.public_key_bytes()[..8]), signer.public_key_bytes().len());
        println!("[BTC]  Schnorr:  {}... ({}B), valid={}", hex::encode(&sig.bytes[..8]), sig.bytes.len(), verifier.verify(b"hello world", &sig)?);
    }
    ...
    // ─── NEO ─────────────────────────────────────────────────────────
    {
        ...
        println!("[NEO]  Pubkey:   {} (P-256 {}B)", hex::encode(&signer.public_key_bytes()[..8]), signer.public_key_bytes().len());
        println!("[NEO]  Sig:      {}... ({}B), valid={}", hex::encode(&sig.bytes[..8]), sig.bytes.len(), verifier.verify(b"hello world", &sig)?);
    }

    println!("\n=== Ed25519 Chains ===\n");

    let ed_seed = hex::decode(
    ...
    // ─── Solana ──────────────────────────────────────────────────────
    {
        ...
        println!("[SOL]  Pubkey:   {} ({}B)", hex::encode(&signer.public_key_bytes()[..8]), signer.public_key_bytes().len());
        println!("[SOL]  Keypair:  {} bytes", signer.keypair_bytes().len());
        println!("[SOL]  Scalar:   {}... (clamped)", hex::encode(&signer.scalar_bytes()[..8]));
        println!("[SOL]  Sig:      {}... valid={}", hex::encode(&sig.bytes[..8]), verifier.verify(b"hello world", &sig)?);
    }
    ...
    // ─── XRP Ed25519 ─────────────────────────────────────────────────
    {
        ...
        println!("[XRP]  Pubkey:   {} (same as SOL ✓)", hex::encode(&signer.public_key_bytes()[..8]));
        println!("[XRP]  Sig:      {}... valid={}", hex::encode(&sig.bytes[..8]), verifier.verify(b"hello world", &sig)?);
    }
    ...
    println!("\n=== BLS Aggregation ===\n");
    {
        ...
        println!("[BLS]  Sig size:    {} bytes (same as single!)", agg.bytes.len());
